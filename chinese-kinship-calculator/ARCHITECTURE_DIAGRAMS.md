# Architecture Diagram: In-Law Relationships

## Resolution Flow Overview

```
┌─────────────────────────────────────────────────────────────┐
│                 resolveRelationship(path, age)              │
│                    Entry Point: Router                      │
└────────────────────┬────────────────────────────────────────┘
                     │
        ┌────────────┼────────────┬───────────────┐
        │            │            │               │
        ↓            ↓            ↓               ↓
    ┌────────┐  ┌──────────┐  ┌────────────┐  ┌──────┐
    │Spouse? │  │Sib+Sp?   │  │Cousin?     │  │Other │
    │Branch  │  │uorspouse │  │Scenario    │  │Blood │
    └────┬───┘  └────┬─────┘  └────┬───────┘  └──┬───┘
         │           │             │             │
    [wife/husb    [sib+wife    [parent+sibling  Map
     + relative]   /husband]     + child]      Lookup
         │           │             │             │
         ↓           ↓             ↓             ↓
    ┌──────────────┐  ┌─────────────────┐  ┌──────────────┐
    │Resolve       │  │Resolve Sibling  │  │ResolveFirst  │
    │SpouseBranch  │  │Spouse           │  │Cousin        │
    │              │  │                 │  │              │
    │A) Parents    │  │• older_br→婆   │  │• 堂哥/堂弟   │
    │B) Siblings   │  │• younger_br→媳  │  │• 表哥/表弟   │
    │C) Single     │  │• older_sis→姐夫 │  │• 堂姐/堂妹   │
    │   token      │  │• younger_sis→妹夫 │  │• 表姐/表妹   │
    └──────────────┘  └─────────────────┘  └──────────────┘
         │                  │                   │
         │                  │                   │
         └──────────┬───────┴───────────────────┘
                    │
                    ↓
         ┌────────────────────┐
         │   Return Result    │
         │   e.g., "岳父"    │
         └────────────────────┘
```

---

## Decision Tree: Path Analysis

```
                    Input: [token1, token2, ...]
                            │
                            ↓
                  ┌─────────────────────┐
                  │ token1 == wife?     │
                  │ token1 == husband?  │
                  └────────┬──────┬─────┘
                   YES     │      │     NO
                   ┌───────┘      └────────┐
                   ↓                       ↓
            ┌─────────────┐          ┌──────────────────┐
            │ Spouse      │          │ token1 is sibling?│
            │ Branch      │          │ token2 is spouse?│
            └─────┬───────┘          └────┬────────┬────┘
                  │                  YES   │        │   NO
           ┌──────┴─────┐            ┌────┘        └─────┐
           ↓            ↓            ↓                    ↓
      ┌────────┐  ┌──────────┐ ┌──────────┐        ┌──────────────┐
      │len>2?  │  │token2==  │ │Sibling+  │        │parent+parent │
      │Too deep│  │father/   │ │spouse    │        │_sib+child?   │
      │        │  │mother?   │ │resolve   │        │→Cousin logic │
      └────┬───┘  │→In-laws  │ │          │        └──────┬───────┘
         YES      │          │ └──────────┘              YES
         │        └──────────┘                           │
         ├───ERROR──→         ↓                          ↓
         │        ┌───────────────┐        ┌──────────────────────┐
         │        │token2 is      │        │resolveFirstCousin()  │
         │        │sibling?       │        │Returns: 堂哥, 表弟   │
         │        │→Calculate     │        │         based on     │
         │        │in-law name    │        │         relativeAge  │
         │        │(wife/husband  │        └──────────────────────┘
         │        │dependent)     │                 │
         │        └─────┬─────────┘                 ↓
         │              ↓          ┌──────────────────────┐
         │          Return         │   Direct Map Lookup  │
         │          in-law         │   relationshipMap    │
         │          (e.g.,岳父)    │   e.g., father→爸爸 │
         │                         └──────────┬───────────┘
         │                                    ↓
         └────────────────┬──────────────────┘
                          ↓
                 ┌──────────────────┐
                 │   Return Result  │
                 │  or "error msg"  │
                 └──────────────────┘
```

---

## State Machine: Relationship Classification

```
                       ┌─────────────────────────┐
                       │  START: [path array]    │
                       └────────────┬────────────┘
                                    │
                        ┌───────────┴─────────────┐
                        │                         │
        ┌───────────────────────┐    ┌──────────────────────────┐
        │  Spouse Branch Check   │    │ Sibling Spouse Check     │
        │  (path[0] is spouse)   │    │ (path=[sib, spouse])     │
        └───────┬───────┬────────┘    └──────┬────────┬──────────┘
             YES│       │NO                  │YES    │NO
                ↓       │                    ↓       │
         ┌─────────┐    │              ┌────────────┐│
         │Spouse   │    │              │Sibling's   ││
         │Branch   │    │              │Spouse      ││
         │Logic    │    │              │Logic       ││
         └────┬────┘    │              └─────┬──────┘│
              │         │                    │       │
              │    ┌────┴────────┐            │       │
              │    │             │            │       │
              │    ↓             ↓            ↓       │
              │  Cousin?      Invalid?    Valid?     │
              │  (3-token     (too deep)   Sibling   │
              │   parent+     ?→Error      Spouse?   │
              │   sib+child)  ↓            Yes↓      │
              │    │         Error msg    Result    │
              │    │                       │
              │    ├──Cousin Logic─→Result │
              │    │                       │
              └────┼───────────────────────┼─────────┘
                   │                       │
                   │ ┌──────────────────────┘
                   │ │
                   ↓ ↓
                ┌─────────────────┐
                │  Blood Mapping  │
                │  Direct Lookup  │
                └────────┬────────┘
                         │
                         ↓
              ┌─────────────────────┐
              │    Return Result    │
              │ (Relationship Name) │
              └─────────────────────┘
```

---

## Data Flow: Wife's Father Example

```
UI Click: 老婆 → 爸爸 → =
          │      │      │
          ↓      ↓      ↓
       Input: ['wife', 'father']
              │
              ↓
    ┌─────────────────────────────┐
    │ resolveRelationship()        │
    │  path=['wife', 'father']    │
    │  age=null                   │
    └──────────┬──────────────────┘
               │
               ↓
    ┌──────────────────────┐
    │ isSpouseBranch()     │
    │ path[0]='wife'?      │
    │ YES → true           │
    └──────────┬───────────┘
               │
               ↓
    ┌──────────────────────────┐
    │ resolveSpouseBranch()    │
    │ path=['wife','father']   │
    └──────────┬───────────────┘
               │
        ┌──────┴──────┐
        ↓             ↓
   Check depth   Check spouse
   2 ≤ 2? ✓       type
        │         t1='wife'
        │         t2='father'
        │         isWife=true
        │         │
        └─────────┤
                  ↓
        ┌──────────────────────────┐
        │ if (t2 === 'father') {   │
        │   return isWife ? '岳父'  │
        │                          │
        │ Result: '岳父'           │
        └──────────┬───────────────┘
                   │
                   ↓
        ┌─────────────────────┐
        │ Display Result:     │
        │     岳父   🔊       │
        │  (+ volume icon)    │
        └─────────────────────┘
```

---

## Data Flow: Older Brother's Wife Example

```
UI Click: 哥哥 → 老婆 → =
          │     │      │
          ↓     ↓      ↓
       Input: ['older_brother', 'wife']
              │
              ↓
    ┌──────────────────────────────┐
    │ resolveRelationship()         │
    │  path=['older_brother','wife']│
    │  age=null                     │
    └──────────┬───────────────────┘
               │
               ↓
    ┌──────────────────────┐
    │ isSpouseBranch()     │
    │ path[0]='older_' ? NO
    └──────────┬───────────┘
               │
               ↓
    ┌──────────────────────────┐
    │ isSiblingSpouse()        │
    │ path.length==2 ? YES     │
    │ path[0] is sibling? YES  │
    │ path[1] is spouse? YES   │
    │ Result: TRUE             │
    └──────────┬───────────────┘
               │
               ↓
    ┌──────────────────────────┐
    │ resolveSiblingSpouse()   │
    │ path=['older_brother',   │
    │       'wife']            │
    └──────────┬───────────────┘
               │
        ┌──────┴──────┐
        ↓             ↓
    t1='older_'   t2='wife'
    brother       isWife=true
        │             │
        └─────────────┤
                      ↓
        ┌──────────────────────────┐
        │ if (isWife) {            │
        │   if (t1==='older_br')   │
        │     return '嫂子'        │
        │                          │
        │ Result: '嫂子'           │
        └──────────┬───────────────┘
                   │
                   ↓
        ┌─────────────────────┐
        │ Display Result:     │
        │     嫂子   🔊       │
        │  (+ volume icon)    │
        └─────────────────────┘
```

---

## Scope Coverage: What's Supported

```
Generation Level Visualization:

  Spouse's Parents      (Supported ✓)
         │ ← wife/husband
         │
  Generation 0          (Supported ✓)
   You ────────── Spouse
         │ ← wife/husband
         │
  Spouse's Siblings     (Supported ✓)
         │
         │ ← brothers/sisters
         │
         └─→ wife/husband
             │
  Your Siblings' Spouse (Supported ✓)
  
  (Max Depth = 2: Spouse + 1 Relative)
```

---

## Unsupported Patterns (By Design)

```
Too Deep (>2 levels):
  [wife, father, brother, son]  ← 4 levels, blocked
  [husband, mother, daughter]   ← 3 levels, blocked
  
Why? Maintains simplicity and avoids exponential complexity

Invalid Combinations:
  [older_brother, husband]      ← Invalid (brother→husband)
  [younger_sister, wife]        ← Invalid (sister→wife)
  [father, wife]                ← Invalid (not direct spouse)
  
Why? Only specific patterns are semantically valid
```

---

## Extension Example: How to Add Spouse's Nieces

If you want to support `wife` → `older_brother` → `daughter`:

```javascript
// Update: resolveSpouseBranch()

function resolveSpouseBranch(path) {
  // OLD: max depth = 2
  if (path.length > 2) {
    return '暂不支持更深层的姻亲关系'
  }
  
  // NEW: allow depth 3
  if (path.length > 3) {  // Changed from > 2
    return '暂不支持更深层的姻亲关系'
  }
  
  const [t1, t2, t3] = path  // Add t3
  
  // ... existing code ...
  
  // NEW: Handle spouse's siblings' children
  if (path.length === 3) {
    const isWife = t1 === 'wife'
    const isHusband = t1 === 'husband'
    const isSibling = t2 === 'older_brother' || 
                      t2 === 'younger_brother' ||
                      t2 === 'older_sister' ||
                      t2 === 'younger_sister'
    const isChild = t3 === 'son' || t3 === 'daughter'
    
    if (isSibling && isChild) {
      // Add logic here
      // e.g., wife + older_brother + son = 大舅子的儿子
    }
  }
  
  return '暂时无法解析'
}
```

This shows how the architecture is extensible!
